...
void loop(){
  ...
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  Serial.println(F("Checando sistemas"));
  Serial.println(F("==================================================="));
  delay(1000);
  Serial.println(F("CHegando velocidade do clock I2C."));
  delay(1000);
  ...
  if(TWBR == 12 && clockspeed_ok){
    Serial.println(F("Velocidade do clock I2C configurada corretamente para 400kHz."));
  }
  else{
    Serial.println(F("Velocidade do clock I2C nao configurada para 400kHz. (ERROR 8)"));
    error = 1;
  }
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("Configurando transmisor"));
    Serial.println(F("==================================================="));
    delay(1000);
    Serial.print(F("Procurando por sinais válidos do transmisor."));
  ...
  if(error == 0){
    delay(2000);
    Serial.println(F("Coloque todos os botoes e subtrims na posicao central dentro de 10 segundos."));
    for(int i = 9;i > 0;i--){
      delay(1000);
      Serial.print(i);
      Serial.print(" ");
    ...
    Serial.print(F("Entrada digital 08 = "));
    Serial.println(receiver_input_channel_1);
    Serial.print(F("Entrada digital 09 = "));
    Serial.println(receiver_input_channel_2);
    Serial.print(F("Entrada digital 10 = "));
    Serial.println(receiver_input_channel_3);
    Serial.print(F("Entrada digital 11 = "));
    Serial.println(receiver_input_channel_4);
    Serial.println(F(""));
    Serial.println(F(""));
  }
  if(error == 0){  
    Serial.println(F("Mova o acelerador para o maximo e novamente para o centro."));
    //Check for throttle movement
    check_receiver_inputs(1);
    Serial.print(F("Acelerador conectado a entrada digital "));
    Serial.println((channel_3_assign & 0b00000111) + 7);
    if(channel_3_assign & 0b10000000)Serial.println(F("Canal invertido = sim"));
    else Serial.println(F("Canal invertido = nao"));
    wait_sticks_zero();
    
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova o stick para simular o movimento de asa direita para cima (rolagem/roll) e retorne para o centro"));
    //Check for throttle movement
    check_receiver_inputs(2);
    Serial.print(F("Rolagem conectado a entrada digital "));
    Serial.println((channel_1_assign & 0b00000111) + 7);
    if(channel_1_assign & 0b10000000)Serial.println(F("Canal invertido = sim"));
    else Serial.println(F("Canal invertido = nao"));
    wait_sticks_zero();
  }
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova o stick para simular o movimento de nariz para cima (arfagem/pitch) e retorne para o centro"));
    //Check for throttle movement
    check_receiver_inputs(3);
    Serial.print(F("Arfagem conectado a entrada digital "));
    Serial.println((channel_2_assign & 0b00000111) + 7);
    if(channel_2_assign & 0b10000000)Serial.println(F("Canal invertido = sim"));
    else Serial.println(F("Canal invertido = nao"));
    wait_sticks_zero();
  }
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova o strick para simular nariz para esquerda (guinada/yaw) e retorne para o centro"));
    //Check for throttle movement
    check_receiver_inputs(4);
    Serial.print(F("Guinada conectado a entrada digital "));
    Serial.println((channel_4_assign & 0b00000111) + 7);
    if(channel_4_assign & 0b10000000)Serial.println(F("Canal invertido = sim"));
    else Serial.println(F("Canal invertido = nao"));
    wait_sticks_zero();
  }
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Mova gentilmente todos os controles simultaneamente por toda sua extencao"));
    Serial.println(F("Quando finalizado, retorne os controles a sua posicao central"));
    //Register the min and max values of the receiver channels
    register_min_max();
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Valores maximos, minimos e centrais encontrados durante a configuracao"));
    Serial.print(F("Valores da entrada digital 08:"));
    Serial.print(low_channel_1);
    Serial.print(F(" - "));
    Serial.print(center_channel_1);
    Serial.print(F(" - "));
    Serial.println(high_channel_1);
    Serial.print(F("Valores da entrada digital 09:"));
    Serial.print(low_channel_2);
    Serial.print(F(" - "));
    Serial.print(center_channel_2);
    Serial.print(F(" - "));
    Serial.println(high_channel_2);
    Serial.print(F("Valores da entrada digital 10:"));
    Serial.print(low_channel_3);
    Serial.print(F(" - "));
    Serial.print(center_channel_3);
    Serial.print(F(" - "));
    Serial.println(high_channel_3);
    Serial.print(F("Valores da entrada digital 11:"));
    Serial.print(low_channel_4);
    Serial.print(F(" - "));
    Serial.print(center_channel_4);
    Serial.print(F(" - "));
    Serial.println(high_channel_4);
    Serial.println(F("Mova o stick 'natiz para cima' e novamente ao centro para continuar"));
    check_to_continue();
  }
  ...
  if(error == 0){
    ...
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("Busca do giroscópio"));
    Serial.println(F("==================================================="));
    delay(2000);
    
    Serial.println(F("Buscando pelo MPU-6050 no endereco 0x68/104"));
    delay(1000);
    if(search_gyro(0x68, 0x75) == 0x68){
      Serial.println(F("MPU-6050 encontrado no endereco 0x68"));
      type = 1;
      gyro_address = 0x68;
    }
    
    if(type == 0){
      Serial.println(F("Buscando pelo MPU-6050 no endereco 0x69/105"));
      delay(1000);
      if(search_gyro(0x69, 0x75) == 0x68){
        Serial.println(F("MPU-6050 encontrado no endereco 0x69"));
        type = 1;
        gyro_address = 0x69;
      }
    }
    ...
    if(type == 0){
      Serial.println(F("Buscando pelo L3G4200D no endereco 0x68/104"));
      delay(1000);
      if(search_gyro(0x68, 0x0F) == 0xD3){
        Serial.println(F("L3G4200D encontrado no endereco 0x68"));
        type = 2;
        gyro_address = 0x68;
      }
    }
    ...
    if(type == 0){
      Serial.println(F("Buscando pelo L3G4200D no endereco 0x69/105"));
      delay(1000);
      if(search_gyro(0x69, 0x0F) == 0xD3){
        Serial.println(F("L3G4200D encontrado no endereco 0x69"));
        type = 2;
        gyro_address = 0x69;
      }
    }
    ...
    if(type == 0){
      Serial.println(F("Buscando pelo L3GD20H no endereco 0x6A/106"));
      delay(1000);
      if(search_gyro(0x6A, 0x0F) == 0xD7){
        Serial.println(F("L3GD20H encontrado no endereco 0x6A"));
        type = 3;
        gyro_address = 0x6A;
      }
    }
    ...
    if(type == 0){
     Serial.println(F("Buscando pelo L3GD20H no endereco 0x6B/107"));
      delay(1000);
      if(search_gyro(0x6B, 0x0F) == 0xD7){
        Serial.println(F("L3GD20H encontrado no endereco 0x6B"));
        type = 3;
        gyro_address = 0x6B;
      }
    }
    ...
    if(type == 0){
      Serial.println(F("Giroscopio nao encontrado!!! (ERROR 3)"));
      error = 1;
    }
      ...
      Serial.println(F(""));
      Serial.println(F("==================================================="));
      Serial.println(F("Configuracoes do registro do giroscopio"));
      Serial.println(F("==================================================="));
  ...
  if(error == 0){
    ...
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("Calibacao do giroscopio"));
    Serial.println(F("==================================================="));
    Serial.println(F("Nao mova o drone!! Calibracao iniciara em 3 segundos"));
    delay(3000);
    Serial.println(F("Calibrando giroscopio, isto levara +/- 8 segundos"));
    ...
    Serial.println(F(""));
    Serial.print(F("Offset do eixo 1="));
    Serial.println(gyro_roll_cal);
    Serial.print(F("Offset do eixo 2="));
    Serial.println(gyro_pitch_cal);
    Serial.print(F("Offset do eixo 3="));
    ...
    Serial.println(F("Levante o lado esquerdo do drone a um angulo de 45 graus dentro de 10 segundos"));
    //Check axis movement
    check_gyro_axes(1);
    if(error == 0){
      Serial.println(F("OK!"));
      Serial.print(F("Deteccao de angulo = "));
      Serial.println(roll_axis & 0b00000011);
      if(roll_axis & 0b10000000)Serial.println(F("Eixo invertido = sim"));
      else Serial.println(F("Eixo invertido = nao"));
      Serial.println(F("Coloque o drone novamente a sua posicao original"));
      Serial.println(F("Mova o stick 'nariz para cima' para continuar"));
      check_to_continue();

      //Detect the nose up movement
      Serial.println(F(""));
      Serial.println(F(""));
      Serial.println(F("Levante o nariz do drone para cima a um angulo de 45 graus dentro de 10 segundos"));
      //Check axis movement
      check_gyro_axes(2);
    }
    if(error == 0){
      ...
      if(pitch_axis & 0b10000000)Serial.println(F("Eixo invertido = sim"));
      else Serial.println(F("Eixo invertido = nao"));
      Serial.println(F("Coloque o drone novamente a sua posicao original"));
      Serial.println(F("Mova o stick 'nariz para cima' para continuar"));
      check_to_continue();
      ...
      Serial.println(F(""));
      Serial.println(F(""));
      Serial.println(F("Gire o drone para a esquerda a um angulo de 45 graus dentro de 10 segundos"));
      //Check axis movement
      check_gyro_axes(3);
    }
    if(error == 0){
      ...
      if(yaw_axis & 0b10000000)Serial.println(F("Eixo invertido = sim"));
      else Serial.println(F("Eixo invertido = nao"));
      Serial.println(F("Coloque o drone novamente a sua posicao original"));
      Serial.println(F("Mova o stick 'nariz para cima' para continuar"));
      check_to_continue();
  ...
  if(error == 0){
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("Teste do LED"));
    Serial.println(F("==================================================="));
    digitalWrite(12, HIGH);
    Serial.println(F("O LED deve estar ligado agora"));
    Serial.println(F("Mova o stick 'nariz para cima' para continuar"));
    check_to_continue();
    digitalWrite(12, LOW);
  }
  ...
  if(error == 0){
    Serial.println(F("==================================================="));
    Serial.println(F("Checagem final da configuracao"));
    Serial.println(F("==================================================="));
    delay(1000);
    if(receiver_check_byte == 0b00001111){
      Serial.println(F("Sinais do receptor OK"));
    }
    else{
      Serial.println(F("Verificacao do canal do receptor falhou!!! (ERROR 6)"));
      error = 1;
    }
    delay(1000);
    if(gyro_check_byte == 0b00000111){
      Serial.println(F("Eixos do giroscopio OK"));
    }
    else{
      Serial.println(F("Verificacao dos eixos do giroscopio falhou!!! (ERROR 7)"));
      error = 1;
    }
  }     
  ...
  if(error == 0){
    ...
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("Gravando informacoes na EEPROM "));
    Serial.println(F("==================================================="));
    Serial.println(F("Encrevendo EEPROM"));
    delay(1000);
    ...
    Serial.println(F("Verificando dados da EEPROM"));
    delay(1000);
    ...
    if(error == 1)Serial.println(F("Verificacao EEPROM falhou!!! (ERROR 5)"));
    else Serial.println(F("Verificacao concluida"));
  }
  
  
  if(error == 0){
    Serial.println(F("Configuracao do drone finalizada."));
    Serial.println(F("Agora voce pode calibrar os ESC's e carregar o codigo YMFC-AL."));
  }
  else{
   Serial.println(F("A configuracao foi abortada devido a um erro."));
   //Serial.println(F("Check the Q and A page of the YMFC-AL project on:"));
   Serial.println(F("www.brokking.net para mais informacoes sobre o erro."));
  }
  while(1);
}
...
void wait_for_receiver(){
  ...
  if(zero == 0){
    error = 1;
    Serial.println(F("."));
    Serial.println(F("Nenhum sinal valido de receptor encontrado!!! (ERROR 1)"));
  }
...
void register_min_max(){
  ...
  Serial.println(F("Medindo amplitudes...."));
  while(zero < 15){
    if(receiver_input_channel_1 < center_channel_1 + 20 && receiver_input_channel_1 > center_channel_1 - 20)zero |= 0b00000001;
    if(receiver_input_channel_2 < center_channel_2 + 20 && receiver_input_channel_2 > center_channel_2 - 20)zero |= 0b00000010;
    if(receiver_input_channel_3 < center_channel_3 + 20 && receiver_input_channel_3 > center_channel_3 - 20)zero |= 0b00000100;
...
void check_gyro_axes(byte movement){
  ...
  if(trigger_axis == 0){
    error = 1;
    Serial.println(F("Nenhum movimento angular detectado nos ultimos 10 segundos!!! (ERROR 4)"));
  }
  else
...
void intro(){
  ...
  delay(1000);
  Serial.println(F(""));
  Serial.println(F("YMFC-AL Programa de configuracao |  TRADUZIDO"));
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  delay(1500);
